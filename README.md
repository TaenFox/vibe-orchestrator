...
- Semantic proof observation не выполняется: reader подтверждает доступность файла и наличие identifier, но не заменяет экспертную оценку. Browser-level проверки DOM/focus/viewport для этого контракта не требуются и в worker-контексте недоступны.
