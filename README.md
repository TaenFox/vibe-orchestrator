...
- **Бюджетный control plane** зафиксирован контрактом [`budget.v1`](BUDGETING.md),
  но enforcement в MVP отсутствует: runtime не создаёт reservations и не
  блокирует запуски по лимитам. `priority` — порядок планирования, а `wip` —
  лимит одновременно выполняемых обычных тикетов, не бюджет.
...
- Версионируемый контракт бюджетного control plane описан в [`BUDGETING.md`](BUDGETING.md);
  budget.v1 принят для реализации, но enforcement в MVP не реализован.
