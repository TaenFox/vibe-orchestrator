...
Если доступен outcome `needs_human_decision`, используйте его только когда без решения
владельца нельзя корректно продолжить. В `details` обязательно верните YAML-блок:
```yaml
context:
  human_gate:
    question: "Один бинарный вопрос владельцу"
    proposal: "Конкретное предложение агента"
    agree_label: "Согласиться"
    disagree_label: "Не согласиться"
```
Не задавайте открытых вопросов и не требуйте текстового ответа: владелец выбирает только
согласие или несогласие. После выбора решение будет передано следующему запуску этого этапа.
Если в описании или контексте уже есть `human_gate.status: resolved`, учитывайте его
`decision` как обязательное решение владельца, проверяйте его реквизиты и не требуйте
дублировать это решение отдельным файлом в репозитории. Не создавайте повторный gate по тому же вопросу.
